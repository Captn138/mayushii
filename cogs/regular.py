...
class Regular(commands.Cog):
    ...
    async def dev(self, ctx):
        ...
        embed.add_field(inline = False, name = "ver 0.3.2", value = "Fixed an error in info command, statically added owner_id in MetaInfo class since client.owner_id doesn't work as expected, will be removed when lib is fixed, moved some functions out of BotBasics class.")
        embed.add_field(inline = False, name = "ver 0.3.1", value = "Changed internal functions, fixed some missing quotes and imports, cleaned unnecessary files.")
        embed.add_field(inline = False, name = "ver 0.3.0", value = "A lot of rewrites after nearly 2 years of downtime, discord-rewrite.py became discord.py, a lot of commands have been simplified, better handeled some errors, removed useless code, added youy command.")
        ...
        embed.add_field(inline = False, name = "Ver 0.2.0", value = "Went from discord.py to discord-rewrite.py. A lot of minimal changes, some upgrades for me to check on the bot, some smoother commands. Added '.clear amount @mention' command. Dropped the autorole command for now. Quite a big update.")
        ...
        embed.add_field(inline = False, name = "Ver 0.1.0", value = "First Beta release; bot now messages the guild owner on join; added dev command.")
        embed.add_field(inline = False, name = "Ver 0.0.2", value = "Added clear command.")
        embed.add_field(inline = False, name = "Ver 0.0.1", value = "First Alpha release, added debug, autorole, okarin, invite, info and help commands; added autoroles on member join, up time and cycling statuses.")
        embed.add_field(inline = False, name = "Coming soon", value = "Music, reactions roles, autoroles, some guild managment and probably more.")
    ...
    async def info(self, ctx):
        ...
        embed.add_field(inline = False, name = "Invite me!", value = MI.get_bot_invite_link())
    ...
    async def help(self, ctx, com = None):
        ...
        if com is None:
            ...
            embed.add_field(inline = False, name = f"{command_prefix}invite", value = "Gives you an invite link to **Je pars à Gé**.")
            ...
            embed.add_field(inline = False, name = f"{command_prefix}autorole", value = "Change the autorole. Manage roles required.")
            embed.add_field(inline = False, name = f"{command_prefix}clear", value = "Clears an amount of messages in the channel. Manage messages required")
            ...
            if com == "tuturu":
                embed.add_field(inline = False, name = f"{command_prefix}tuturu", value = f"Displays youy. {__class__.youy_limit} minutes timeout.")
                embed.add_field(inline = False, name = "Usage:", value = f"{command_prefix}youy")
            elif com == "youy":
                embed.add_field(inline = False, name = f"{command_prefix}youy", value = "Plays a nice \"Tuturu\" in your voice channel. Must be connected to a voice channel.")
                ...
                embed.add_field(inline = False, name = f"{command_prefix}autorole", value = "Manage roles lock! Changes the role given automatically when a new member joins the server.")
                ...
                embed.add_field(inline = False, name = "Warning:", value = "You will be asked for a new role name after. You can cancel the operation.")
            elif com == "kick":
                embed.add_field(inline = False, name = f"{command_prefix}kick", value = "Kick members lock! Kicks a member from the channel.")
                embed.add_field(inline = False, name = "Usage:", value = f"{command_prefix}kick <@member> (reason)")
            elif com == "ban":
                embed.add_field(inline = False, name = f"{command_prefix}ban", value = "Ban members lock! Bans a member from the channel")
                embed.add_field(inline = False, name = "Usage:", value = f"{command_prefix}ban <@member> (reason)")
            elif com == "unban":
                embed.add_field(inline = False, name = f"{command_prefix}unban", value = "Ban members lock! Unbans a member from the channel")
                embed.add_field(inline = False, name = "Usage:", value = f"{command_prefix}unban <Member#xxxx>")
            elif com == "ping":
                embed.add_field(inline = False, name = f"{command_prefix}ping", value = "Calculates the bot's ping and sends it in the channel.")
            elif com == "dev":
                embed.add_field(inline = False, name = f"{command_prefix}dev", value = "Shows the development panel, which contains all the updates history and the upcoming updates.")
            elif com == "clear":
                embed.add_field(inline = False, name = f"{command_prefix}clear", value = "Clears a specified amount of messages in the channel. If used with a @mention, clears an amount of messages from the specified user. Warning : limit is 500 messages")
                ...
                embed.add_field(inline = False, name = f"{command_prefix}info", value = "Gives you infos about the bot: Creator, uptime, number of guilds and a link to get it.")
                embed.add_field(inline = False, name = "Aliases:", value = f"{command_prefix}i")
            elif com == "invite":
                embed = discord.Embed(title = f"{self.client.user.name}'s help menu", description = "Specific command:", color = 0xa1ee33)
                embed.add_field(inline = False, name = f"{command_prefix}invite", value = "Sends an invite to the Creator's discord guild.")
            else:
                await ctx.message.delete()
                return
